...
# Licence : MIT
# Repository : https://github.com/VOTRE-USERNAME/yunohost-mail-security-audit
# Description : Génère un rapport HTML quotidien de sécurité mail
# ===================================================================
...
# ⚠️ CONFIGURATION - MODIFIEZ CETTE LIGNE ⚠️
...
# Statistiques
TOTAL_ATTEMPTS=$(grep "$TODAY" "$LOG_FILE" 2>/dev/null | grep "auth=0/1" | wc -l)
EXTERNAL_AUTH=$(grep "$TODAY" "$LOG_FILE" 2>/dev/null | grep "sasl_method=" | grep -vE "192.168|10\.|172\.(1[6-9]|2[0-9]|3[01])|127\.0\.0\.1" | wc -l)
SENT_MAILS=$(grep "$TODAY" "$LOG_FILE" 2>/dev/null | grep "status=sent" | wc -l)
BANNED_TOTAL=0
...
if [ "$EXTERNAL_AUTH" -eq 0 ] && [ "$SENT_MAILS" -lt 200 ] && [ "$TOTAL_ATTEMPTS" -lt 100 ]; then
    ...
    GLOBAL_COLOR="#10b981"
    GLOBAL_ICON="✓"
    SUBJECT="[OK] Rapport Sécurité Mail - $HOSTNAME - $(date +%d/%m/%Y)"
else
    GLOBAL_STATUS="ATTENTION"
    GLOBAL_COLOR="#f59e0b"
    GLOBAL_ICON="⚠"
    SUBJECT="[ALERTE] Rapport Sécurité Mail - $HOSTNAME - $(date +%d/%m/%Y)"
fi

# Top 5 IPs attaquantes
TOP_IPS=$(grep "auth=0" "$LOG_FILE" 2>/dev/null | \
    grep -oE "[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+" | \
    sort | uniq -c | sort -rn | head -5)

# Top utilisateurs légitimes
TOP_USERS=$(grep "$TODAY" "$LOG_FILE" 2>/dev/null | \
    grep "sasl_username=" | \
...
<head>
    ...
    <style>
        * {
        ...
        .status-banner {
            padding: 30px 40px;
            background: GLOBAL_COLOR_PLACEHOLDER;
            color: white;
            text-align: center;
            font-size: 24px;
            font-weight: 700;
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Analyse quotidienne du serveur HOSTNAME_PLACEHOLDER</p>
        </div>
        
        <div class="status-banner">
            GLOBAL_ICON_PLACEHOLDER STATUT : GLOBAL_STATUS_PLACEHOLDER
        </div>
        ...
        <div class="content">
            <div class="info-grid">
                <div class="info-card">
                    <h3>Serveur</h3>
                    <p>HOSTNAME_PLACEHOLDER</p>
                </div>
                <div class="info-card">
                    <h3>IP Publique</h3>
                    <p>PUBLIC_IP_PLACEHOLDER</p>
                </div>
                <div class="info-card">
                ...
                <div class="info-card">
                    <h3>Fail2ban</h3>
                    <p>FAIL2BAN_STATUS_PLACEHOLDER</p>
                </div>
            </div>
            
            <div class="stats-grid">
                <div class="stat-card danger">
                    <div class="stat-number">TOTAL_ATTEMPTS_PLACEHOLDER</div>
                    <div class="stat-label">Tentatives d'attaque</div>
                </div>
                <div class="stat-card warning">
                    <div class="stat-number">BANNED_TOTAL_PLACEHOLDER</div>
                    <div class="stat-label">IPs bannies</div>
                </div>
                <div class="stat-card info">
                    <div class="stat-number">SENT_MAILS_PLACEHOLDER</div>
                    <div class="stat-label">Mails envoyés</div>
                </div>
                <div class="stat-card success">
                    <div class="stat-number">EXTERNAL_AUTH_PLACEHOLDER</div>
                    <div class="stat-label">Connexions externes</div>
                </div>
            </div>
            ...
            <div class="section">
                <h2 class="section-title">🎯 Top 5 des IPs Attaquantes</h2>
            ...
            <div class="section">
                <h2 class="section-title">👥 Connexions Légitimes</h2>
                <div class="table-container">
                    <table>
                        <thead>
                            <tr>
            ...
            <div class="section">
                <h2 class="section-title">🚫 IPs Actuellement Bannies</h2>
                <div class="table-container">
                    <table>
                        <thead>
                            <tr>
        ...
        <div class="footer">
            <p>Rapport généré automatiquement par <strong>Mail Security Audit</strong></p>
            <p>Serveur YunoHost • Fail2ban • Postfix • Dovecot</p>
        </div>
    </div>
</body>
</html>
...
ALERTS_HTML=""
if [ "$EXTERNAL_AUTH" -eq 0 ] && [ "$SENT_MAILS" -lt 200 ] && [ "$TOTAL_ATTEMPTS" -lt 100 ]; then
    ALERTS_HTML='<div class="alert alert-success"><strong>✓ Tout est OK !</strong> Aucune anomalie détectée. Le serveur fonctionne normalement.</div>'
else
    if [ "$EXTERNAL_AUTH" -gt 0 ]; then
        ALERTS_HTML+='<div class="alert alert-warning"><strong>⚠ Authentifications externes détectées</strong> : '$EXTERNAL_AUTH' connexions depuis l extérieur du réseau local.</div>'
    fi
    if [ "$TOTAL_ATTEMPTS" -gt 100 ]; then
        ALERTS_HTML+='<div class="alert alert-danger"><strong>✗ Volume d attaques élevé</strong> : '$TOTAL_ATTEMPTS' tentatives d authentification échouées aujourd hui.</div>'
    fi
    if [ "$SENT_MAILS" -gt 200 ]; then
        ALERTS_HTML+='<div class="alert alert-warning"><strong>⚠ Volume de mails élevé</strong> : '$SENT_MAILS' mails envoyés aujourd hui. Vérifier si spam.</div>'
    fi
fi
...
[ -z "$TOP_IPS_HTML" ] && TOP_IPS_HTML='<tr><td colspan="3" style="text-align:center; color: #10b981;">Aucune attaque détectée aujourd hui ✓</td></tr>'
...
[ -z "$TOP_USERS_HTML" ] && TOP_USERS_HTML='<tr><td colspan="3" style="text-align:center;">Aucune connexion aujourd hui</td></tr>'
...
if [ -n "$ALERT_EMAIL" ]; then
    if command -v mutt &> /dev/null; then
        ...
        echo "$(date '+%Y-%m-%d %H:%M:%S') - Rapport HTML envoyé à $ALERT_EMAIL" >> /var/log/mail_audit.log
    else
    ...
    echo "ERREUR : Veuillez configurer ALERT_EMAIL dans le script (ligne 14)"
fi
